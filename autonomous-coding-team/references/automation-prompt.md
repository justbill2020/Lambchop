...
Regenerate docs/[project slug]/dashboard-data.json and docs/[project slug]/dashboard.html during setup and after every run. The dashboard must be grounded in real state, scheduled work, progress, backoff, validation, leases, blockers, commits, and next actions. Do not hand-write decorative status that cannot be traced back to workflow evidence.
