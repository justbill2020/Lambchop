...
Plan the next phase and execute exactly one eligible work item unless the workflow says there is no source-backed work. If no eligible queued item exists, inspect the scheduled work plan and create the next bounded work item from its backlog seeds or from newly discovered source evidence before declaring no-work. Red-team your own plan before and after implementation so you do not get tunnel vision around one file or one symptom. Review what may have been left incomplete by the previous phase, then continue one eligible incomplete work item until it is implemented, verified, documented when needed, committed locally, and reflected in state and progress.

After completing or blocking the active item, run the planner loop before applying backoff: reconcile state with repository reality, inspect the scheduled work plan, add the next dependency-safe work item when source-backed work remains, and record the task-generation or no-work decision in progress.
