...
- Shared capabilities: check or install required upstream skills once into the shared Codex skills area. Superpowers is required for workflow discipline; Huashu Design is required for dashboard/app UI, prototype, visual direction, and design critique work.
- Source check-in: record the Lambchop source commit used for setup or upgrade so later hooks/automation runs can compare it with the current Lambchop source and repair stale deployments.
