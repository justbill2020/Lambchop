...
Regenerate the repo-local visual dashboard after setup and every automation run. The dashboard must use real state, scheduled work, progress, backoff, validation, leases, blockers, commits, and next-action data.
...
4. Let Lambchop create or repair `WORKFLOW.md`, `docs/{project-slug}/state.json`, `progress.md`, `backoff.json`, `scheduled-work-plan.md`, `dashboard-data.json`, `dashboard.html`, and the weekly Codex cron automation.
...
Validate the upgraded setup by parsing JSON ledgers, checking for unresolved placeholders outside templates, confirming the dashboard opens locally, and recording the upgrade evidence in progress.md.
```
