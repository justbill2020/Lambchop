...
Lambchop is also self-hosted: the active autonomous coding agent may run inside this repository while this repository is the autonomous coding agent system being changed. Treat that recursion as a first-class constraint. Workflow, hook, schedule, dashboard, issue-tracker, prompt, and source-commit changes must leave durable source-of-truth evidence, not merely status text.
...
9. GitHub Issues is the default durable issue/PRD discussion tracker for this repository; `docs/lambchop/state.json` remains the executable automation queue.
...
- `project chat execution`: a normal interactive Codex chat in this repository where Bill reports a feature request, bug, regression, rough need, or "this is broken" style issue outside a scheduled automation run.
...
- `GitHub Issues tracker`: the default external issue tracker for Lambchop and GitHub-backed Lambchop-managed repos. Use `gh issue ...` commands and the conventions in `docs/agents/issue-tracker.md`; do not treat dashboard control requests as issue-tracker execution.
- `self-hosting recursion`: the condition where Lambchop's active autonomous coding agent is changing the Lambchop repo that defines that same autonomous agent. Recursive maintenance must record real workflow/source-of-truth deltas and validation evidence.

## Allowed Actions
Automation may inspect the repo, create worktrees, create local `codex/lambchop-{work_item_key}` branches, edit skill/docs/templates for the active item, run validation commands, commit locally, update state/progress/schedule ledgers, and use GitHub Issues through `gh issue ...` for issue tracking when the action is workflow-authorized.

## Forbidden Actions
Automation must not publish branches unless `docs/lambchop/state.json` explicitly sets `project.autonomy_policy.may_push=true`. When `may_push=true`, publishing is expected after each coherent local commit for completed work items (unless validation is failing or workflow safety blocks publishing for another reason). Automation must not open pull requests, deploy, modify production configuration, use non-GitHub external trackers, delete or revert user work, do implementation work directly in the main checkout after setup, overwrite another live lease, or mark work done without validation evidence. GitHub issue creation, comments, labels, and reads are allowed only through the documented `gh issue` workflow; closing issues still requires explicit workflow or Bill authorization.

## Project Chat Execution
When Bill reports a new need, feature request, bug, regression, vague problem, or "this is broken" issue in an ordinary project chat, that chat session may investigate, plan, and implement directly when the work is bounded enough to leave validated source-of-truth evidence in the same turn.

If Bill explicitly requests workflow/scheduling/hook/shared-capability (maintenance) work in the current chat, record that explicit override by setting `docs/lambchop/state.json` `project.chat_policy.mode` to `maintenance` for the duration of the maintenance work, then reset it to `default` after the changes are validated, committed, and (when allowed) pushed.
...
The chat should diagnose enough to choose between direct execution and deferral. It may inspect relevant files, run diagnostic or reproduction checks, identify likely ownership, edit production code when the change is bounded, and update workflow ledgers. If the work is too broad, blocked on a decision, unsafe to finish in one turn, or better handled by the recurring automation, create or update bounded work items in `docs/lambchop/state.json` with source references, acceptance criteria, implementation notes, validation expectations, exclusive scope, shared scope, blockers if any, and a clear `next_step`. For GitHub-tracked work, create or update the corresponding GitHub issue with `gh issue ...` and start every AI-generated issue/comment with `> *This was generated by AI during triage.*`

When the chat implements directly, it must still leave source-of-truth evidence: update progress/dashboard/backoff/state, validate the behavior, and commit coherent completed work when appropriate. When the chat defers instead of finishing, it must record why, verify the parked weekly anchor, unpause the project automation when it is paused or record that it was already active, trigger a scheduler-visible run-now for the same automation, and record the queued task, automation status, trigger result, and dashboard/progress/backoff evidence.
...
If Bill explicitly disables or removes repo-local hooks because they are breaking the autonomous coding team, record `hooks.status` as `disabled_by_operator` and do not reinstall or repair Lambchop-owned hooks until Bill explicitly re-enables them. In that state, `.codex/hooks.json` may intentionally contain no active hook handlers; this is not malformed, unavailable, or a repair target. Continue with the automation-only contract and keep scheduler/progress/dashboard evidence current.

Hooks are an active-session quality layer. They load Lambchop context on session start, reinforce diagnose-first behavior for user prompts, block clearly forbidden local actions before supported tool calls, remind Codex to record evidence after risky or failed tool calls, and continue a turn when required Lambchop completion evidence is missing. For feature/bug chats, the hooks must steer the agent toward bounded direct implementation when the work can be finished safely in the current turn with source-of-truth evidence, or toward task creation plus automation status and scheduler-visible run-now handoff when the work is being deferred. For GitHub repos where push is explicitly enabled or requested, the Stop hook must require commit and push evidence before completion; it must not blindly publish when workflow safety still forbids pushing. Hooks are not a complete enforcement boundary and they do not replace the parked weekly cron automation or scheduler-visible completion trigger.
